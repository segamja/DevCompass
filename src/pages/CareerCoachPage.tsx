...
export default function CareerCoachPage() {
  const { messages, sendMessage, isSending, sendError } = useCareerCoach()
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setLocalError(err instanceof Error ? err.message : t('errors.apiRequestFailed'))
    }
  }
        ...
        <div className="absolute bottom-0 left-0 right-0 p-margin-desktop bg-gradient-to-t from-surface-bg via-surface-bg/95 to-transparent">
          <div className="max-w-4xl mx-auto space-y-4">
            {(localError || sendError) && (
              <div className="p-3 rounded-xl bg-error-container text-on-error-container font-body-sm text-center">
                {localError || sendError}
              </div>
            )}
